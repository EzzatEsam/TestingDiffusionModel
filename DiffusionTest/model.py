...
class Encoder(nn.Module):
    ...
    def forward(self, x, time_vec):
        x1 = self.down1(x, time_vec)
        x2 = self.down2(x1, time_vec)
        x3 = self.down3(x2, time_vec)
        x4 = self.down4(x3, time_vec)
        x5 = self.down5(x4, time_vec )
...
class Decoder(nn.Module):
    def __init__(self, in_channels, time_dim):
        ...
        self.up1 = UpModule(in_channels, 512, time_dim , has_attention=True)
        self.up2 = UpModule(1024, 256, time_dim)
        self.up3 = UpModule(512, 128, time_dim)
        self.up4 = UpModule(256, 64, time_dim)
