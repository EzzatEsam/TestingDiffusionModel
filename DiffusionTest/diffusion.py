...
class Diffusion:
    ...
    def generate_sample(self, model: T.nn.Module, n=10, label: int | None = None):
        with T.no_grad():
            model.eval()
            img = T.randn((1, self.img_channels, self.img_size, self.img_size)).to(
                self.device
            )
            imgs = []
            for i in reversed(range(self.time_steps)):
                if not self.is_hg:
                    ...
                    predicted_noise = model(img, i).sample
